...
const IMPORTER_PROJECT_PREFIXES = [ROUTES.importerProjects];
const IMPORTER_ALLOWED_ROLES = [USER_ROLES.importer];

export default function ImporterProjectsLayout({
  children,
}: {
  children: ReactNode;
}) {
  return (
    <RoleProtectedPortalShell
      ...
      protectedPrefixes={IMPORTER_PROJECT_PREFIXES}
      role={USER_ROLES.importer}
      title="Importer project access"
    >
      {children}
    </RoleProtectedPortalShell>
  );
